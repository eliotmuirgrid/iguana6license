#ifndef __DRG_FILTER_DECRYPT_H__
#define __DRG_FILTER_DECRYPT_H__
...
// Description:
//
// Copied from STMCRYPTO2/DRGfilterDecrypt.
//
...
#include "DRGconstant.h"

class DRGfilterDecryptPrivate;

enum DRGfilterDecryptErrorCode
{
   DRG_KEY_TOO_LARGE = 100,
   DRG_KEY_NO_DECRYPTION_KEY = 101,
   DRG_KEY_MISMATCH = 102,
   DRG_GENERAL_ERROR = 103
};

class COL_DLL DRGfilterDecrypt : 
   public COLfilter
{
public:
   DRGfilterDecrypt( COLsink* pNext_ );
   virtual ~DRGfilterDecrypt();
   ...
   //error code in exceptions can be DRGfilterDecryptErrorCode
...
private:
   DRGfilterDecryptPrivate* pMember;
   
   DRGfilterDecrypt( DRGfilterDecrypt& Orig ); // not allowed
   DRGfilterDecrypt& operator=( DRGfilterDecrypt& Orig ); // not allowed
};
