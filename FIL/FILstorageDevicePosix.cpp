...
#include <stdio.h>
#if defined (CHM_BSD) || defined (CHM_MAC) || defined (CHM_MAC_INTEL)
   #include <fstab.h>
   #include <sys/param.h>
   #include <sys/mount.h>
   #include <sys/ucred.h>
...
void FILstorageDevicePrivate::mountEntry()
{
   ...
   COLPOSTCONDITION(FileHandle != NULL);
   ...
   while (true)
   {
      ...
      if (strcmp(DevicePath.c_str(),MountEntry.mnt_mountp) == 0)
      {
         Label = MountEntry.mnt_special;
         FileSystemName = MountEntry.mnt_fstype;
         ...
         if (statvfs(DevicePath.c_str(),&FileSystemInfo) != 0)
         {
            COL_THROW_STRERROR("statvfs failed.");
         }

         SerialNumber = 0;
         MaxFileNameLength = FileSystemInfo.f_namemax;
         SpaceForMe = (COLuint64)FileSystemInfo.f_bavail * FileSystemInfo.f_frsize;
         SpaceForEveryOne = (COLuint64)FileSystemInfo.f_bfree * FileSystemInfo.f_frsize;
         SpaceUsed = (COLuint64)(FileSystemInfo.f_blocks - FileSystemInfo.f_bfree) * FileSystemInfo.f_frsize;
