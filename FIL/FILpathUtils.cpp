...
COLstring FILpathSimplify(const COLstring& Path) {
   ...
   // this (hopefully) shouldn't be a problem.
   //
   ...
   // a big block of data.
   COLsimpleBuffer Buffer(Path.size() + 1);

   char* pOutputStart = (char*)Buffer.data();
   ...
   enum {
      ST_START,  // Beginning of a directory name, we're at the first char 
                 // after the slash.

      ST_DATA,   // Any old directory name content which isn't something we
...
#endif
      return FILpathSimplify(Path);
   } if (WorkingDirectory.is_null()) {
      WorkingDirectory = FILcurrentWorkingDir();
   }
   COL_VAR2(Path, WorkingDirectory);
...
COLstring FILpathAppend(const COLstring& PathA, const COLstring& PathB) {
   ...
   if (PathA.is_null()) {
      return FILpathSimplify(PathB);
   }
   COLstring Result(PathA);
   FILaddPathSeparator(Result);
   Result.append(PathB);
   return FILpathSimplify(Result);
}
   ...
   while (--pRead >= pBegin)
   {
      switch (*pRead)
      {
         ...
         case '/':
            ...
            return FILpathSimplify(COLstring(pBegin, pRead - pBegin));
      }
   }
   return COLstring();
}
...
void FILpathParentDirList(COLvector<COLstring>& DirList, const COLstring& FilePath) {
