...
COLsimpleBuffer SFIbase64::encode(const COLuint8* pSourceBuffer, COLuint32 SourceLength)
{
   COLsimpleBuffer ResultBuffer( ( (SourceLength+2)/3 ) * 4 + 1);
   COLuint8* pResultBuffer = ResultBuffer.data();
...
COLsimpleBuffer SFIbase64::decode(const char* pSourceBuffer)
{
   COLsimpleBuffer ResultBuffer( ((strlen(pSourceBuffer)+5)/4) * 3);
   COLuint8* pResultBuffer = ResultBuffer.data();

   // concatinate '===' to the source to handle unpadded base64 data */   
   COLvector<char> Source;
   ...
   ResultBuffer.resize(ResultBuffer.size() - (ResultBuffer.end() - pResultBuffer));
   return ResultBuffer;
}
...
void SFIbase64::encodeNonTerminated(const COLuint8* pSourceBuffer, COLuint32 BufferLength, COLsimpleBuffer& OutputBuffer)
{
   if (!BufferLength)
   {
      return;
   ...
   OutputBuffer.resize( OutputBuffer.size()-1 );
}

void SFIbase64::decodeNonTerminated(const COLuint8* pSourceBuffer, COLuint32 BufferLength, COLsimpleBuffer& OutputBuffer)
{
   if (!BufferLength)
   {
      return;
