...
public:
   ...
   // returns a COLsimpleBuffer containing the encoded null-terminated
   // string
   static COLsimpleBuffer encode(const COLuint8* pSourceBuffer, COLuint32 BufferLength);

   // assumes encoded pSourceBuffer is null terminated and returns 
   // a buffer containing the decoded data 
   static COLsimpleBuffer decode(const char* pSourceBuffer);

   //writes to the provided simplebuffer the base64 encoded value. does not null terminate
   static void encodeNonTerminated(const COLuint8* pSourceBuffer, COLuint32 BufferLength, COLsimpleBuffer& OutputBuffer);

   //writes to the provided simplebuffer the base64 decoded value. does not null terminate
   static void decodeNonTerminated(const COLuint8* pSourceBuffer, COLuint32 BufferLength, COLsimpleBuffer& OutputBuffer);
