...
COL_DLL void COLthrowWin32Error
(
   long ErrorCode, // e.g. COLlastError() or ::GetLastError()
   const char* Description=NULL,
   COLuint32   LineNumber=-1, 
   const char* FileName=NULL
);

COL_DLL long COLlastError(); // wrapper around Win32 LastError

#endif // WIN32
// The next macro is very handy.  It will build a COLerror
// exception and throw it.  Any time that you see a WIN32 API
...
// i.e. 
// if ( WIN32 API Call Returns Failure Code)
// {
...
{\
   ...
   COLthrowWin32Error(ErrorCode, _ErrorSink.string().c_str(), __LINE__, __FILE__);\
}
...
COL_DLL COLstring COLformatWin32Error(long ErrorCode);

// This is a thread safe wrapper for strerror - it invokes the thread
// safe version of strerror which is strerror_r and ensures that a big
// enough buffer is allocated to take the error message.
// This is also safer than calling strerror directly since under linux
// certain values of ErrorNumber result in segmentation faults (argh).
COL_DLL COLstring COLstrerror(int ErrorNumber);

class COL_DLL COLerror : public COLrefCounted
{
public:
   enum COLerrorCode 
   {
   ...
   COLerror(const COLstring& Message="", int Code=COLerror::PreCondition);
...
COL_DLL COLostream& operator<<(COLostream& Stream, const COLerror& Error);

// This is good if you have an ANSI C function that sets the errno variable.
// this macro will generate a COLerror object with a nice error message using COLstrerror
// i.e.
...
#define COL_THROW_STRERROR(Message) {\
   ...
   Stream << Message << ' ' << COLstrerror(ErrorCode) << '.'; \
...
// The callback allows for extra information to be added to COLPRECONDITION
// and COLPOSTCONDITIONs without much effort. Note that the callback must not
// throw an exception.
typedef void (*COLassertCallback)(COLostream&);

void COLassertDefaultCallback(COLostream&);

class COLassertSettings
{
public:
   static void setAbortOnAssert(bool);
   static bool abortOnAssert();

   static void setCallback(COLassertCallback);
   static COLassertCallback callback();  
...
#define COL_THROW_IMPL(Condition, Message, AssertType) \
   ...
   {\
      ...
      COLostream ColErrorStream(_ErrorSink);\
      ColErrorStream << Message; \
      if (COLassertSettings::abortOnAssert()) \
      {\
         COLabort();\
      }\
      COLassertSettings::callback()(ColErrorStream); \
      throw COLerror(_ErrorSink.string(), __LINE__, __FILE__, AssertType); \
   }
