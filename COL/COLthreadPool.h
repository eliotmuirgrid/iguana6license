...
class COLthreadPool {
 public:
   ...
   //
   COLthreadPool(int minThreads = 1, int maxThreads = 1);
   ...
   // The COLrunnable task input parameter must have been heap
   ...
   //
   void scheduleTask(COLrunnable* task);

   //
   // Same as other scheduleTask() except this copies the COLrunnable argument.
   //
   void scheduleTask(const COLrunnable& task);
 ...
 private:
   ...
   // all scheduleTask() COLrunnable are run within this worker thread function
   static void* _workerThreadRunner(void* arg);

   friend class COLdeferredRunnable;
   ...
   COLthreadQueue<COLrunnable*> _controllerThreadQueue;
   COLthreadQueue<COLrunnable*> _taskQueue;
};
