...
void COLdll::freeLibrary()
{
   ...
   if(::FreeLibrary((HINSTANCE)DllHandle) == FALSE)
   {
      ...
      throw COLerror(COLformatWin32Error(ErrorNumber), ErrorNumber);
   }
   DllHandle = NULL;
}
...
{  
   ...
   else
   {
      ...
      ErrorString = COLformatWin32Error(LastErrorNumber);
...
void* COLdll::getProcAddress(const char* ProcName)
{
   ...
   void* pFunction = NULL;
   pFunction = ::GetProcAddress((HINSTANCE)DllHandle, ProcName);
   ...
   if (pFunction == NULL)
   {
      ...
      {
         throw COLerror(COLformatWin32Error(LastError) + " while trying to load " + COLstring(ProcName), LastError);
      }
      else
      {
