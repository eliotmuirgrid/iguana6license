...
#include <COL/COLplaceIterator.h>
...
public:
   ...
   static COL_DLL int compare(const Type& A, const Type& B) {
      if (B < A) {
         return 1;
      } else if (A < B) {
         return -1;
...
public:
   ...
   typedef COLplaceIteratorAssociative< COLmap<KeyType,Type,CompareType> >       TIterator;
   typedef COLplaceIteratorAssociativeConst< COLmap<KeyType,Type,CompareType> >  TIteratorConst;
   ...
   COLmap(const COLmap& that) : COLavlTree<KeyType,Type>(CompareType::compare) {
      ...
      for (TIteratorConst itr(that) ; itr.next() ; )
         me.add(itr.key(), itr.value());
   }
