...
//
//   Unified with IPAvar - will have to revisit memory efficiency later.
//-----------------------------------------------------------------------------
...
//
// TODO: will have to restructure IPArule and COLspan later to not penalize
//       COLvar general use.
//
class IPArule;
...
public:
   ...
   COLspan(const COLspan& that) {
      start_  = that.start_;
      length_ = that.length_;
   }
   COLspan& operator=(const COLspan& that) {
      start_  = that.start_;
      length_ = that.length_;
      return *this;
   }
   bool operator==(const COLspan& that) const {
      return start_  == that.start_
          && length_ == that.length_;
   }
   inline void set(const char* str, int len) {
      start_  = str;
      length_ = len;
   }
   inline void reset() {
...
public:
   ...
   typedef COLvector<COLvar>                         COLvarVec;
   ...
   // Implementation of this method is in IPGvar.cpp.
   // If you call COLvar::parse() in your application then you must specify makefile
   // MODULES in this order: IPG IPA REX COL
   ...
   COLstring asEscapedString(int encoding=StrEncCpp) const;
   ...
   int       getInt    (const COLstring& key) const;
   ...
   COLstring json(int nesting=0, int encoding=StrEncCpp) const;

   // Output COLvar as JSON with embedding IPArule comments.
   COLstring jsonAnnotatedWithRules(int nesting = 0) const;
   ...
   // Get/set the IPArule that generated this node.
   // The rule pointer is not owned by this instance.
   // The rule pointer is copied in copy ctor and operator=().
   void setRule(const IPArule* p)    { rule_ = p;    }
   const IPArule* rule() const       { return rule_; }

   COLstring ruleName() const;
  
   // Get/set parsed span.
   // The span char pointer is not owned by this instance.
   // Span is copied in copy ctor and operator=().
   void setSpan(const char* str, int len) { span_.set(str, len); }
   void setSpan(const COLspan& span)      { span_ = span; }
   const COLspan& span() const            { return span_; }
   const char* spanStart() const          { return span_.start(); }
   int spanLength() const                 { return span_.length(); }

   // Null out rule_ and span_.
   // Implementation of this method is in IPAvar.cpp
   void stripParseArtifacts();
...
private:
   ...
   // Win64 and Linux64 => 56 bytes due to 8 byte alignment of span_.
   Payload        u_;
   COLspan        span_; // not owner of its data
   const IPArule* rule_; // not owner
   Type           type_;
};
