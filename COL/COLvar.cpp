...
// Slightly non-standard - emits special characters as \xHH
// because \Uxxxx unicode escape sequences cannot handle binary
// bytes correctly.
//
static void AppendJsonString(COLstring* Out, const COLstring& In, bool WithQuotes, int encoding) {
   COL_FUNCTION(AppendJsonString);
         ...
         switch (Ch) {
            ...
            default  : if (0x20 <= Ch && Ch < 0x7F) {
                          ...
                          if (Ch < 0x20 || Ch == 0x7F) {
                             ...
                             BufferCount += ::sprintf(Buffer + BufferCount, "\\u%04X", Ch);
                          ...
                          BufferCount += ::sprintf(Buffer+BufferCount, "\\x%02X", Ch);
                       }
         }
...
COLstring COLvar::asEscapedString(int encoding) const {
   COLstring out;
   AppendJsonString(&out, asString(), false, encoding);
   return out;
}
...
int COLvar::getInt(const COLstring& key) const
{
   if (const COLvar* child = get(key)) {
      return child->asInt();
   }
...
int COLvar::getInt(int idx) const
{
   if (const COLvar* child = get(idx)) {
      return child->asInt();
   }
...
{
   COLstring out;
   AppendJsonString(&out, in, false, encoding);
   return out;
}
...
COLstring COLvar::json(int N, int encoding) const
{
   ...
   case None:
      ...
      break;
   case String:
      AppendJsonString(&s, *(COLstring*)u_.colstring_, true, encoding);
      break;
   case Integer:
      {
         const int len = ::sprintf(buf, "%d", u_.int_);
         s.set(buf, len);
      }
      break;
   case Boolean:
   ...
   case Map:
      {
         if (u_.map_ && u_.map_->size() > 0) {
            ...
            for (COLvarMap::iterator it = u_.map_->begin(); it != u_.map_->end(); ++it) {
               ...
               isFirst = false;
               AppendJsonString(&s, it->first, true, encoding);
               s += ':';
               if (N) s += ' ';
...
#if defined(CHM_WIN) || defined(CHM_MAC_INTEL) || defined(CHM_LINUX)
