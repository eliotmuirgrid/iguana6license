#ifndef __COL_VECTOR_H__
#define __COL_VECTOR_H__
//---------------------------------------------------------------------------
// Copyright (C) 1997-2014 iNTERFACEWARE Inc.  All Rights Reserved
//
// Module: COLvector
//
// Description: Dynamic vector class patterned after STL vector
//
...
// Define to preserve the legacy COLvector legacy behavior to use memmove to
// move elements in the vector rather rather invoking the copy constructor
// and in-place destructor. Some parser classes depend on this COLvector
...
// stored in COLvectors to ones whose bit patterns can safely be shifted.
...
// more info.
//#define COLvector_LEGACY_MEMMOVE_BEHAVIOR

// Uncomment this to enable extra debugging for destructed elements.
//#define COLvector_MEMSET_DESTRUCTED

#include "COLerror.h"
#include <new>

#ifdef COLvector_MEMSET_DESTRUCTED
#  include <memory.h>
#  define COLvector_CLEAR_ELEMENT_MEMORY(m_) memset((m_),0xd7,sizeof(*(m_)))
#else
#  define COLvector_CLEAR_ELEMENT_MEMORY(m_) 
#endif
...
public:
   ...
   explicit COLvector(size_type n, const T& v = T())
   {
      if (n<0) n = 0;
      size_ = 0;
      capacity_ = 0;
   ...
   COLvector(const COLvector& that)
   : size_(0), capacity_(0)
   {
      heap_ = 0;
      *this = that;
   ...
   COLvector& operator=(const COLvector& that)
   {
      if (this != &that)
      {
         clear();
   ...
   void clear()
   {
      ...
      {
         heap_[i].~T();
         COLvector_CLEAR_ELEMENT_MEMORY(&heap_[i]);
      }
      deleteHeap();
      capacity_ = 0;
      size_ = 0;
   ...
   iterator erase(iterator pos)
   {
      ...
      if (begin() <= pos && pos < E)
      {
#ifdef COLvector_LEGACY_MEMMOVE_BEHAVIOR
         pos->~T();
         COLvector_CLEAR_ELEMENT_MEMORY(pos);
         memmove((void*)pos, pos+1, (E-pos-1)*sizeof(T));
#else
         for (iterator p = pos; p < E;)
         {
            p->~T();
            COLvector_CLEAR_ELEMENT_MEMORY(p);
            p++;
            if (p < E)
            {
               new ((void*)(p-1)) T(*p);
