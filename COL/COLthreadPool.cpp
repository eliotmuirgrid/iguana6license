...
// Description
//
// Dynamic thread pool to schedule COLrunnable instances.
//-----------------------------------------------------------------------------
...
//
class COLdeferredRunnable : public COLrunnable {
 public:
   COLdeferredRunnable(COLthreadPool& threadPool, COLrunnable* runnable)
   : _threadPool(threadPool) {
      _runnable = runnable;
   }
   virtual ~COLdeferredRunnable() {
      delete _runnable;
   }
   void run() {
      // Note: it is perfectly valid to push a null _runnable here.
      // A null _runnable instructs a worker thread to quit.
      _threadPool._taskQueue.put(_runnable);
      _runnable = 0; // consumed by _taskQueue, set to 0
   }
   COLdeferredRunnable* clone() const {
      return 0; // not supported
   }
 private:
   COLdeferredRunnable(const COLdeferredRunnable&);
   COLdeferredRunnable& operator=(const COLdeferredRunnable&);

   COLthreadPool& _threadPool;
   COLrunnable*   _runnable;
};
...
COLthreadPool::COLthreadPool(int minThreads, int maxThreads)
{
   ...
   _threadWhereCreated = COLthread::getCurrentThreadID();
...
{
   COL_METHOD(COLthreadPool::~COLthreadPool);
   COLASSERT(_threadWhereCreated == COLthread::getCurrentThreadID());
   ...
   // all threads are finished reading the task queue at this point
   // so we may clean up any possible pending tasks.
...
// thread pool takes ownsership of heap allocated task
void COLthreadPool::scheduleTask(COLrunnable* task) {
   COL_METHOD(COLthreadPool::scheduleTask);
   if (!_immediateShutdownRequested) {
      // defer running task until it is within controller thread
      _controllerThreadQueue.put(new COLdeferredRunnable(*this, task));
   }
   else {
      // if the task is not scheduled it must be deleted.
      delete task;
   }
}

// clone the task and push the heap allocated copy
void COLthreadPool::scheduleTask(const COLrunnable& task)
{
   if (!_immediateShutdownRequested) {
      COLrunnable* runnable = task.clone();
      COLASSERT(runnable);
...
void COLthreadPool::shutdown(COLthreadPool::ShutdownType shutdownType)
{
   ...
   _controllerThreadQueue.put(new COLdeferredRunnable(*this, 0));
...
void COLthreadPool::_addThreads(int numberOfNewThreads)
{
   ...
   COL_TRC("added " << numberOfNewThreads
      << " worker thread(s) for a total of " << getTotalNumberOfThreads());
}
...
void COLthreadPool::addThreads(int numNewThreads)
{
   ...
   if (!_immediateShutdownRequested) {
      _controllerThreadQueue.put(COLnewRunnable(this, &COLthreadPool::_addThreads, numNewThreads));
   }
}

int COLthreadPool::getTotalNumberOfThreads() const
{
   // this is only a snapshot!
   return _threadJoinQueue.size();
}

int COLthreadPool::getPendingTaskCount() const
{
   // this is only a snapshot!
...
void* COLthreadPool::_controllerThreadRunner(void* arg)
{
   ...
   for (;;) {
      // Note: this loop MUST NOT break upon _immediateShutdownRequested
      // or it will not shut down its worker threads properly.

      // block until we have a task to run
      COLrunnable* task = This->_controllerThreadQueue.take();
...
void* COLthreadPool::_workerThreadRunner(void* arg)
{
   ...
   // Loop until immediate shutdown is requested
   // or a null task is read from the task queue.
   // An immediate shutdown may leave tasks in queue unexecuted.

   while (!This->_immediateShutdownRequested)
   {
      // block until we have a task to run
      COLrunnable* task = This->_taskQueue.take();
      ...
      try {
         ...
         COL_ERR("_workerThreadRunner: runnable exception caught");
      }
