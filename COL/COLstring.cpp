...
#else
   sprintf(Result, "%lu", (unsigned long)Value);
   return Result;
#endif
}
...
#else
   sprintf(Result, "%llu", (unsigned long long)Value);
   return Result;
#endif
}
...
COLstring COLtoAscii(const COLstring& String, char Marker) {
   ...
   if (Marker == '%') { 
      // We are about to sprintf, so we need to escape a percent sign
   ...
   for (int i = 0; i < Max; i++) {
      ...
      if (0x20 <= Ch && Ch < 0x7F) {
         ...
         COLstring Tank;
         Tank.assign(3, '\0');
         ::sprintf(Tank.get_buffer(), Pattern.c_str(), Ch);
         Ascii.append(Tank);
      }
   }
   COL_VAR(String);
...
int COLstring::toWideCharacter( const char* String, COLsimpleBuffer& Output )
{
   int WideCharacterLength = 0;
   if( String )
   {
      int BytesInInput = strlen(String);
      //this will convert the string from the multibyte string to wide char in
      //the current locale
      WideCharacterLength = mbstowcs( 0, String, 0 );
      if (WideCharacterLength == -1)
      {
         COL_ERROR_STREAM("Unable to convert multibyte string to wide character, locale may be incorrect: "
                           << String, COLerror::SystemError);
      }
      Output.resize((WideCharacterLength + 1) * sizeof(wchar_t));//make space for null character
      ::memset(Output.data(),0,Output.size()); //clear to ensure that the null character is there
      WideCharacterLength = mbstowcs( reinterpret_cast<wchar_t*>( Output.data() ), String, WideCharacterLength );
      if (WideCharacterLength == -1)
      {
         COL_ERROR_STREAM("Unable to convert multibyte string to wide character, locale may be incorrect: "
                           << String, COLerror::SystemError);
