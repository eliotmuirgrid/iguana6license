...
public:
   ...
   // Output is in bytes, cast the data() to a wchar_t to get the wchar_t result.
   // Returns the wchar_t length.
   static int toWideCharacter(const char* cstr, COLsimpleBuffer& OutputBuffer);

   // Splits the COLstring using the token provided as the delimiter and returns the values in the provided strings. 
   // rpslit uses the last occurence of the token in the string  as the split point.
   ...
   // Returns a new COLstring with consecutive occurrences of the specified character removed
