...
COLindex COLhash<COLstring>::defaultHash(const COLstring& Key)
{
   ...
   if (Key.size())
   {
      ...
      // on the Solaris and HP-UX platforms it is necessary to make a copy of Key before hashing,
      // otherwise a Bus Error and Core Dump can result due to unaligned access
#if defined(CHM_SOLARIS) || defined(CHM_HPUX) || defined(CHM_HPUX_ITANIUM)
      // this is inefficient, but not worth optimizing for these obsolete platforms
      COLstring Key2(Key.c_str());
      const char* pStr = Key2.c_str();
#else
