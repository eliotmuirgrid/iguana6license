...
//
class COL_DLL COLlogFile : public COLsink
{
public:
   COLlogFile()
   {
...
static COLvector<int*>    gLineEnableVector;   // guarded by getTraceLogMutex()
...
: COLostream(m_String)
{
   ...
   time_t now = (time_t)currentTime.tv_sec;
   {
      ...
      if (now != gLastSecondsSinceEpoch)
      {
         ...
         localtime_r(&now, &tm);
#endif
         ::sprintf(gLastTimeBuffer, "%02d:%02d:%02d.", tm.tm_hour, tm.tm_min, tm.tm_sec);
   ...
   //
   int PrefixLength = millisStart + ::sprintf(LinePrefix+millisStart, "%03d %012lx %-28s ",
      nowMillis, COL_getCurrentThreadID(), module);
...
void COLlog::reset(int argc, const char** argv){
   ...
   for (int loop = 0; loop < 2; ++loop)
   {
      ...
      { 
         ...
         COLvector<int*>::iterator Iter = gLineEnableVector.begin();
...
void COLlog::dump(const char* module, const void* pData, int nbytes) const
{
   ...
   for (int i = 0; i < nbytes; ++i) {
      ...
      if (++c >= s_COL_DUMP_WIDTH || i+1 >= nbytes) {
         ...
         char RowPrefix[16];
         sprintf(RowPrefix, "%6d: ", i/s_COL_DUMP_WIDTH*s_COL_DUMP_WIDTH);
...
#endif // COL_ENABLE_CONDITIONAL_LOGGING
