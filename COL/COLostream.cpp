...
{
   char Buffer[32];
   int CountOfChar = sprintf(Buffer, pMember->isDec()? "%i": "%X", Number);
...
{
   char Buffer[32];
   int CountOfChar = sprintf(Buffer, pMember->isDec()? "%u": "%X", Number);
   pMember->pSink->write((COLuint8*)Buffer, CountOfChar);
   return *this;
}
...
{
   char Buffer[32];
   int CountOfChar = sprintf(Buffer, pMember->isDec()? "%hi": "%hX", Number);
   pMember->pSink->write((COLuint8*)Buffer, CountOfChar);
   return *this;
}
...
{
   char Buffer[32];
   int CountOfChar = sprintf(Buffer, pMember->isDec()? "%hu": "%hX", Number);
   pMember->pSink->write((COLuint8*)Buffer, CountOfChar);
   return *this;
}
...
{
   char Buffer[64];
   int CountOfChar = sprintf(Buffer, pMember->isDec()? "%lli": "%llX", Number);
   pMember->pSink->write((COLuint8*)Buffer, CountOfChar);
   return *this;
}
...
{
   char Buffer[64];
   int CountOfChar = sprintf(Buffer, pMember->isDec()? "%llu": "%llX", Number);
   pMember->pSink->write((COLuint8*)Buffer, CountOfChar);
   return *this;
}
...
COLostream& COLostream::operator<<(COLfloat32 Number)
{
   ...
   int CountOfChar = sprintf(Buffer, "%f", Number);
   pMember->pSink->write((COLuint8*)Buffer, CountOfChar);
   return *this;
}
...
COLostream& COLostream::operator<<(COLfloat64 Number)
{
   ...
   int CountOfChar = sprintf(CharBuffer, "%f", Number);
...
COLostream& COLostream::operator<<(const void* Pointer)
{
   ...
   // int CountOfChar = sprintf(CharBuffer, "%p", Pointer);
#ifdef CHM_WIN
   int CountOfChar = sprintf(CharBuffer, "0x%I64x", Pointer);
#else
   int CountOfChar = sprintf(CharBuffer, "0x%lx", (unsigned long)Pointer);
#endif
   pMember->pSink->write(CharBuffer, CountOfChar);
   return *this;
}
...
COL_DLL void __set_indent(COLostream& OutStream, const COLstring& IndentContent)
{
   OutStream.setIndentContent(IndentContent);
}

COL_DLL COLostream& indent(COLostream& OutStream)
{
   OutStream.indent();
   return OutStream;
}

COL_DLL COLostream& unindent(COLostream& OutStream)
{
   OutStream.unindent();
   return OutStream;
}

COL_DLL COLostream& reset_indent(COLostream& OutStream)
{
...
COL_DLL COLostreamManip* set_indent(const COLstring& TagName)
{
