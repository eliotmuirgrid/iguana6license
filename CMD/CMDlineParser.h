...
public:
   ...
   COLuint32 countOfExtraArgument() const;

   // Retrieves the value of an extra argument
   const COLstring& extraArgument(COLuint32 ArgumentIndex) const;
   ...
   const COLuint32 countOfFile() const;

   // Returns the file at the given index
   const COLstring& fileName(const COLuint32 FileIndex) const;

   // Checks whether FlagName is present in the command line input.
   // If a flag is present, but has not yet been registered using addFlagWithoutArgument, 
   // it is registered.
   // Cannot be used to register flags with arguments
   // DEPRECATED - use addFlagWithArgument instead.
   bool flagPresent(const COLstring& FlagName, 
   ...
   //    addFlagWithArgument(FlagName, ArgumentName);
   //    flagPresent(FlagName, Description);
   ...
   // DEPRECATED - use parsingErrorsPresent() instead
   bool unknownFlagPresent(COLostream& ErrorStream) const;
   ...
   bool findExtraParam(const COLstring& Parameter, COLuint32& ParamIndexOut) const;
