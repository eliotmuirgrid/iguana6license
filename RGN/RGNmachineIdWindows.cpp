...
#if defined(WIN32) && defined(CHM_64)
...
{
   // #6820 Due to weird IE 7 bug we dynamically link to netapi32.dll
   COLdll NetApi32;
   try
   {
   ...
   {
      // TODO not really cool assuming the error code here.
      return RGNmakeDummyId();
   }

   RGNnetCall NetCall = (RGNnetCall)NetApi32.getProcAddress("Netbios");
   ...
   // TODO - does it really make sense to concatenate all the IDs we find together?
   for (int NetWorkCardIndex = 0;
        NetWorkCardIndex < lenum.length;
        NetWorkCardIndex++)
   {
      memset( &Ncb, 0, sizeof(Ncb) );
      Ncb.ncb_command = NCBRESET;
      Ncb.ncb_lana_num = lenum.lana[NetWorkCardIndex];
      ...
      sprintf(Buffer, "%02x%02x%02x%02x%02x%02x",
         (unsigned char)Adapter.adapt.adapter_address[0],
         (unsigned char)Adapter.adapt.adapter_address[1],
         (unsigned char)Adapter.adapt.adapter_address[2],
         (unsigned char)Adapter.adapt.adapter_address[3],
         (unsigned char)Adapter.adapt.adapter_address[4],
         (unsigned char)Adapter.adapt.adapter_address[5]);
...
)
{
   ...
   char Buffer[16];
   sprintf(Buffer, "%04x", Hash);
   Hash = (Hash + 1231231L) % 2;
   sprintf(Buffer + 4, "%04x", Hash);
...
void RGNcheckLicense()
{
   ...
   RGNlicenseLevel LicenseLevel
      = RGNregistrationLevel(RGNmachineId(),
                             RGNregistrationCode(),
                             RGNexpiryDate(),
                             RGNdoesNotExpire());

   COL_VAR(LicenseLevel);
   if ((LicenseLevel == RGN_EXPIRED) ||
       (LicenseLevel == RGN_NOT_LICENSED))
   {
...
bool RGNisLicensed()
{
   ...
   bool IsLicensed = LicenseLevel != RGN_EXPIRED &&
      LicenseLevel != RGN_NOT_LICENSED;
   COL_VAR(IsLicensed);
   return IsLicensed;
}
