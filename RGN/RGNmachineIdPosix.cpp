...
COLstring RGNmacAddress() {
   ...
   close(Socket);
   RGNprintMACAddress(MACAddress.get_buffer(), (COLuint8*) &Buffer.ifr_ifru.ifru_hwaddr.sa_data);
   COL_VAR(MACAddress);
   return MACAddress;
}

// END of defined(CHM_LINUX) ******************************

#elif defined(CHM_BSD) || defined(CHM_MAC)
   ...
   for (pByte = TempBuffer; pByte < TempBuffer + InterfaceConfig.ifc_len ; ) {
      ...
      if (pInterfaceRequest->ifr_addr.sa_family == AF_LINK) {
         ...
         if (pSocketAddress->sdl_type == IFT_ETHER) {
            ...
            COLstring MACAddress(RGN_MACHINE_ID_MAC_ADDRESS_LENGTH, '\0');
            RGNprintMACAddress(MACAddress.get_buffer(), (COLuint8*) LLADDR(pSocketAddress));
...
// END of defined(CHM_BSD) || defined(CHM_MAC) *******************************************
...
void RGNprintMACAddress(char* pBuffer, unsigned char* pMACAddress)
{
...
COLstring RGNlegacyMachineId()
{
   ...
   sprintf(MachineId.get_buffer(), "%8x", (unsigned)gethostid());
   MachineId.stripAll(' ');
...
)
{
   ...
   sprintf(Buffer, "%04x", (unsigned)Hash);
   Hash = (Hash + 1231231L) % 2;
   sprintf(Buffer + 4, "%04x", (unsigned)Hash);
