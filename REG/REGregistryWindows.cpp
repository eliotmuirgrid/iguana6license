...
public:
   ...
   (
      ...
      REGregistry::REGstringType StringType
   );
...
REGregistryPrivate::REGregistryPrivate(HKEY iRootKey)
: LazyWrite(true), RootKey(iRootKey), CurrentKey(NULL), KeyIndex(0)
{
   // do nothing
...
void REGregistryPrivate::checkResult(LONG Result) const
{
   ...
   {
      COL_ERROR_STREAM(COLformatWin32Error(Result).c_str(), COLerror::SystemError);
   }
}

void REGregistryPrivate::writeString
(
   const COLstring& Name,
   const COLstring& Value,
   REGregistry::REGstringType StringType
...
const COLstring REGregistryPrivate::readString(const COLstring& Name, const char* pDefault, REGregistry::REGstringType* pActualType)
{
   ...
   {
      FinalBuffer = (const char*)Buffer.data();
      if(pActualType)
      {
...
bool REGregistry::readEnumString(COLstring& Name, COLstring& Value, DWORD* pOutType)
{
   ...
   do
   {
      ...
      DWORD Result = RegEnumValue(pMember->CurrentKey, 
                                  pMember->KeyIndex, 
                                  Name.get_buffer(), 
                                  &SizeOfBuffer,
                                  NULL,
...
bool REGregistry::readEnumKey(COLstring& Name)
{
   ...
   DWORD Result = RegEnumKeyEx(pMember->CurrentKey, 
                               pMember->KeyIndex, 
                               Name.get_buffer(), 
                               &SizeOfBuffer,
                               NULL,
